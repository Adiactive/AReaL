...
from areal.engine.megatron_utils.packed_context_parallel import preprocess_packed_seqs_context_parallel
        ...
        **kwargs,
        ...
        if self.pre_process:
            ...
            if cu_seqlens is not None:
                combined_embeddings, packed_seq_params = preprocess_packed_seqs_context_parallel(
                    combined_embeddings.transpose(0, 1).contiguous(), cu_seqlens
                )
                combined_embeddings = combined_embeddings.transpose(0, 1).contiguous()
            elif packed_seq_params is not None:
                # Fallback if someone else passes packed_seq_params without cu_seqlens, 
                # though in AReaL we should use cu_seqlens.
                pass
        ...
        cu_seqlens = kwargs.get("cu_seqlens", None)
        if cu_seqlens is not None:
            position_ids, _ = preprocess_packed_seqs_context_parallel(
                position_ids.permute(1, 2, 0).contiguous(), cu_seqlens
            )
            position_ids = position_ids.permute(2, 0, 1).contiguous()
            attention_mask = None
        elif packed_seq_params is not None:
            attention_mask = None
