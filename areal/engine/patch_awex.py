...
def _patch_gpu_sub_command():
    import awex

    original_method = awex.util.gpu.get_gpu_status

    def get_gpu_status_by_env() -> str:
        if os.environ.get("AWEX_PRINT_STATUS_BY_COMMAND", "0") == "1":
            return original_method()
        return "Skip run sub command,run it by using export AWEX_PRINT_STATUS_BY_COMMAND=1."

    awex.util.gpu.get_gpu_status = get_gpu_status_by_env
